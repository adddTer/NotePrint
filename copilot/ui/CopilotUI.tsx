...
export interface CopilotUIProps {
    ...
    onThinkingStatusChange?: (isThinking: boolean) => void;
}
...
export const CopilotUI: React.FC<CopilotUIProps> = ({ 
    ...
    useEffect(() => {
        if (typeof onThinkingStatusChange === 'function') {
            onThinkingStatusChange(isThinking);
        }
    }, [isThinking, onThinkingStatusChange]);
    ...
    // Override progressText if copilotState has a specific statusMsg or is active
    if (!isOpen && context?.copilotState?.isActive) {
        progressText = context.copilotState.statusMsg || '编辑中...';
    }

    if (!isOpen) {
        const showProgress = isThinking || context?.copilotState?.isActive;
        return (
            <motion.button
                layout
                ...
                className={`fixed bottom-20 right-4 md:bottom-24 md:right-6 h-14 bg-blue-600 hover:bg-blue-700 text-white shadow-2xl flex items-center justify-center z-50 ring-4 ring-white dark:ring-neutral-900 overflow-hidden transition-all duration-300 ${showProgress ? 'w-48 rounded-full' : 'w-14 rounded-full'}`}
            >
                {showProgress ? (
                    <div className="flex items-center gap-2 px-4 w-full">
                        <div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin shrink-0" />
                        <span className="text-xs font-bold tracking-widest truncate">{progressText}</span>
                    </div>
                ) : (
            ...
            >
                ...
                <div className="flex-1 flex overflow-hidden bg-neutral-50/50 dark:bg-neutral-950/50 relative">
                    ...
                    <div className="flex-1 flex flex-col relative min-w-0 overflow-hidden">
                        ...
                        {/* Task State Display */}
                        {isThinking && currentThread?.taskState && (
